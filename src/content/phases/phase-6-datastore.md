...
При выключении сервера Roblox вызывает **и** `PlayerRemoving` для каждого игрока, **и** `BindToClose`. Без защиты данные записывались бы дважды — не сломаны, но зря тратит лимиты запросов. В коде выше это решено флагами `serverClosing` / `shutdownSaved`: при выключении каждый игрок сохраняется ровно один раз.
